...
module.exports = {
	...
	triviaEvent : function(tchannel, Discord, storage, bot) {
		var embed = new Discord.RichEmbed();
		this.triviachannel = tchannel;
		embed.setColor(0x00FFFF);
		embed.setTitle("Trivia Event Started");
		this.triviachannel.sendEmbed(embed);
		this.askQuestion();
	},
	...
	askQuestion : function() {
		var question;
		do {
			question = utils.getRandomIntInclusive(0, (this.questions.length-1));
		}
		while (this.completedquestions.includes(question));
		this.triviachannel.sendMessage(this.questions[question].question);
		this.completedquestions.push(question);
	},

	checkAnswer : function(msg, storage) {
		if (msg.channel.id == this.triviachannel.id) {
			var question = this.completedquestions[this.completedquestions.length-1];
			var result = msg.content.toLowerCase() == this.questions[question].answer || msg.content.toLowerCase() == this.questions[question].altanswer;
			if (result) {
				msg.reply("has the right answer. It was " + this.questions[question].answer);
				var points = this.trivialeaderboard.find(function(el) {
					return el.usernamediscrim == msg.author.username+"#"+msg.author.discriminator;
				});
				if (points == undefined) {
					this.trivialeaderboard.push({ usernamediscrim : msg.author.username + "#" + msg.author.discriminator, score : 1 });
				}
				else {
					var point = this.trivialeaderboard.find(function(el) {
						return el.usernamediscrim == msg.author.username+"#"+msg.author.discriminator;
					});
					point.score++;
				}
				if (this.completedquestions.length >= this.questions.length) {
					this.triviachannel.sendMessage("Trivia Complete! This channel will now be deleted.");
					this.displayLeaderboard(msg.guild);
					this.resetTrivia(storage, msg, this.triviachannel);
				}
				else {
					this.askQuestion();
				}
	...
	isWaiting : function() {
		return this.completedquestions.length > 0;
	},

	displayLeaderboard : function(guild) {
		for (var src = 0; src < this.trivialeaderboard.length; src++) {
			// try to bubble up
			for (var dst = src - 1; dst >= 0; dst--) {
				if (this.trivialeaderboard[src].score > this.trivialeaderboard[dst].score) {
					// swap
					var tmp = this.trivialeaderboard[dst];
					this.trivialeaderboard[dst] = this.trivialeaderboard[src];
					this.trivialeaderboard[src] = tmp;
					src--;
				}
			}
		}
		var count = Math.min(10, this.trivialeaderboard.length);
		var leaderboardoutput = "";
		for (var i = 0; i < count; i++) {
			leaderboardoutput += (i+1) + ". " + this.trivialeaderboard[i].usernamediscrim + " \n \t Score: " + this.trivialeaderboard[i].score + "\n";
		}
		guild.defaultChannel.sendMessage(":clipboard: **" + guild.name + "'s Trivia Leaderboard**\n```" + leaderboardoutput + "```");
		this.trivialeaderboard.length = 0;
	},

	resetTrivia : function(storage, msg, tchannel) {
		this.completedquestions.length = 0;
